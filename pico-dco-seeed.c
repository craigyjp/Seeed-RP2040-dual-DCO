...
uint8_t RANGE_PWM_SLICES2[NUM_VOICES];
...
void voice_task() {
    if (midi_pitch_bend != last_midi_pitch_bend || DETUNE != LAST_DETUNE || portamento || FM_VALUE != LAST_FM || CONT1_VALUE != LAST_CONT1 || OCT != LAST_OCT) {

        last_midi_pitch_bend = midi_pitch_bend;
        LAST_DETUNE = DETUNE;
        LAST_FM = FM_VALUE;
        LAST_CONT1 = CONT1_VALUE;
        LAST_OCT = OCT;

        for (int i=0; i<NUM_VOICES; i++) {
            float freq;
            if (i == 0){
                freq = get_freq_from_midi_note(VOICE_NOTES[i]);
            }
            if (i == 1){
                freq = get_freq_from_midi_note(VOICE_NOTES[i]) * (1 + (pow(-1, i) * DETUNE));  
            }  
            freq += FM_VALUE * FM_INTENSITY; // Add linear frequency modulation
            freq = freq += CONT1_VALUE * CONT1_INTENSITY; // Add controller1 modulation
...
void adc_task() {
    ...
    // voltage offset. The range of the Pico's ADCs is 3.3V, so a fixed 
    // offset of 1.65V is needed.
    int signed_rawcont = rawcont - (1 << 11);
    CONT1_VALUE = (float) signed_rawcont / (float) (1 << 11);
    #endif
}
